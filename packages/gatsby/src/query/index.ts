  ...
  return {
    ...
    pageQueryIds:
      grouped?.page
        ...
        ?.filter(Boolean) || [],
    fragmentQueryIds: grouped?.fragment || [],
  }
}
...
function createFragmentQueryJob(
  state: IGatsbyState,
  queryId: string
): IQueryJob | undefined {
  const fragmentName = queryId.substring(4)

  const fragmentDef = state.fragments.get(fragmentName)
  if (!fragmentDef) {
    console.log(`cant find fragment "${fragmentName}"`)
    return undefined
  }

  const component = state.components.get(fragmentDef.componentPath)

  if (!component) {
    console.log(`cant find component "${fragmentDef.componentPath}"`)
    return undefined
  }

  const { componentPath, context } = fragmentDef
...
export async function processFragmentQueries(
  queryIds: IGroupedQueryIds["fragmentQueryIds"],
  { state, activity, graphqlRunner, graphqlTracing }
): Promise<void> {
  const processedQueries = await processQueries<string>({
    queryIds,
    createJobFn: createFragmentQueryJob,
